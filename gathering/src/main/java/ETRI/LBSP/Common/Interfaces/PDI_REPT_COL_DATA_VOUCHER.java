...
//Type: REPT_COL_DATA
public class PDI_REPT_COL_DATA_VOUCHER extends PDI_MSG
{
...
//	public short			B22_ColPoint_CNT;
	public ArrayList<PDI_REPT_COL_DATA_VOUCHER_PAYLOAD> B21_Payload;
	public ArrayList<PDI_REPT_COL_DATA_VOUCHER_PAYLOAD_NOMAGNET> B22_Payload;


	private PDI_REPT_COL_DATA_VOUCHER()
	{
		B1_GID = new byte[B1_GID_LEN];
		B9_ColStartP_CVADDR = new PDI_CIVIC_ADDR();
		B21_Payload = new ArrayList<PDI_REPT_COL_DATA_VOUCHER_PAYLOAD>();
		B22_Payload = new ArrayList<PDI_REPT_COL_DATA_VOUCHER_PAYLOAD_NOMAGNET>();
	}

	public PDI_REPT_COL_DATA_VOUCHER(String devSerial )
	{
...
//			B9_ColStartP_CVADDR.FD7_CVADDR_LV6 = SimpleGatheringActivity.mPlace;
	...
	public byte[] packet()
	{
		...
		for ( PDI_REPT_COL_DATA_VOUCHER_PAYLOAD e : B21_Payload )
		{
...
//		buf.putShort(B22_ColPoint_CNT);
	...
	public byte[] packet_nomagnet()
	{
		...
		packetLen_excl_Header 	 += 8 + 8 + 4 + 1 + 4; //추가됨
		packetLen_excl_Header 	 += 1 + 2;
		for ( PDI_REPT_COL_DATA_VOUCHER_PAYLOAD_NOMAGNET e : B22_Payload )
		{
			packetLen_excl_Header += e.packetLen();
		}
		packetLen_excl_Header += 8+8;
...
//		buf.putDouble(B21_ColStartP_TM_Y);
	...
	// magnetic
	public static PDI_REPT_COL_DATA_VOUCHER decode (ByteBuffer packet )
	{
		int offset = 0;
		PDI_REPT_COL_DATA_VOUCHER z = new PDI_REPT_COL_DATA_VOUCHER();
		...
		offset += PDI_REPT_COL_DATA_VOUCHER.B1_GID_LEN;
		...
		z.B6_ColStartP_Y = tmpBuf.getFloat(0);
		...
		tmpBuf = ByteBuffer.allocate(8);
		tmpBuf.put ( buf, offset, 8 );
		z.B5_ColStartP_X = tmpBuf.getFloat(0);
...
//			System.out.println ( "  - PDI_REPT_COL_DATA_PAYLOAD [#" + i + "] SIZE=" + PDI_REPT_COL_DATA_PAYLOAD_SIZE );
			...
			PDI_REPT_COL_DATA_VOUCHER_PAYLOAD p = PDI_REPT_COL_DATA_VOUCHER_PAYLOAD.decode ( tmpBuf );
	...
	// no magnetic
	public static PDI_REPT_COL_DATA_VOUCHER decode2 (ByteBuffer packet )
	{
		int offset = 0;
		PDI_REPT_COL_DATA_VOUCHER z = new PDI_REPT_COL_DATA_VOUCHER();
		...
		tmpBuf = ByteBuffer.allocate(8);
		tmpBuf.put ( buf, offset, 8 );
		z.B5_ColStartP_X = tmpBuf.getFloat(0);//  - (int)ResultList_Path_Act.BaseX;
		...
		tmpBuf = ByteBuffer.allocate(8);
		tmpBuf.put ( buf, offset, 8 );
		z.B6_ColStartP_Y = tmpBuf.getFloat(0);// - (int)ResultList_Path_Act.BaseY;
...
//		offset += 8;
		...
		for ( int i = 1; i <= z.B20_ColPoint_CNT; i++ )
		{
			...
			tmpBuf = ByteBuffer.allocate(PDI_REPT_COL_DATA_PAYLOAD_SIZE);
			tmpBuf.put ( buf, offset, PDI_REPT_COL_DATA_PAYLOAD_SIZE );
			PDI_REPT_COL_DATA_VOUCHER_PAYLOAD_NOMAGNET p = PDI_REPT_COL_DATA_VOUCHER_PAYLOAD_NOMAGNET.decode ( tmpBuf );
