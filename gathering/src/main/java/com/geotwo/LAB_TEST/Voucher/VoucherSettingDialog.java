...
public class VoucherSettingDialog extends Dialog {
    private Context context;
    private View.OnClickListener setpEvent;

    public VoucherSettingDialog(Context context, View.OnClickListener viewstepEvt) {
    ...
    private ImageView  frequency_24Img, frequency_5Img, log_message_img;
    private TextView stepLength;
    private String mLogMessage = "false";
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.voucher_setting_dialog);
        ...
        RelativeLayout log_layout = (RelativeLayout) findViewById(R.id.log_layout);
        log_layout.setOnClickListener(btnEvent);
        log_message_img = (ImageView) findViewById(R.id.log_message_img);

        mLogMessage = isLOG(context);
        if("true".equals(mLogMessage)) {
            setClickImg(log_message_img, true);
        } else {
            setClickImg(log_message_img, false);
        }
...
//                setGsensor(context, !isChecked);
                ...
                setClickImg(frequency_24Img, false);
                setClickImg(frequency_5Img, true);
            } else if(id == R.id.log_layout) {
                // SSID 메시지 노출여부
                WataLog.d("mLogMessage=" + mLogMessage);
                if("true".equals(mLogMessage)) {
                    // 취소하기
                    mLogMessage = "false";
                    setClickImg(log_message_img, false);
                    setStepLength(context, "false");
                } else {
                    // 설정하기
                    mLogMessage = "true";
                    setClickImg(log_message_img, true);
                    setStepLength(context, "true");
                }
            }
...
//                .setTitle(context.getString(R.string.step_setting))
                .setItems(arr, new OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
...
//    }
    ...
    public static String isLOG(Context context) {
        SharedPreferences pref = context.getSharedPreferences("log_message", 0);
        return pref.getString("log_message", "false");
    }

    public static void setStepLength(Context context, String message) {
        SharedPreferences pref = context.getSharedPreferences("log_message", 0);
        SharedPreferences.Editor prefEditor = pref.edit();
        prefEditor.putString("log_message", message);
        prefEditor.commit();
    }
